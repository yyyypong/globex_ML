...
% 1) Perform kernel perceptronaz on different datasets
...
function K=RBFKernel(X,Y)
    ...
    n=size(Y,2);
    K=zeros(m,n);
    distances = pdist2(X, Y, 'euclidean');
    for i=1:m
        for j=1:n
            % Your code here to calculate RBF kernel
            K(i,j)=exp(-(distances(i,j)^2)/(2*sigma^2));  % RBF
        end
    end
end
...
function A=kernelPerceptron(X,Y,kernel)
	...
	for it=1:nt       
        ...
        if kernel == "RBF"
            yhat = sgmd(A * RBFKernel(X, x));
        elseif kernel == "linear"
            yhat = sgmd(A * LinearKernel(X, x));
        end
